...
//        if (!languageVersionSettings.supportsFeature(LanguageFeature.ExpectedTypeFromCast)) return
        ...
        builder.addSubtypeConstraint(typeInSystem, expectedType, ConstraintPositionKind.EXPECTED_TYPE_POSITION.position())
    }
    ...
    private fun getBinaryWithTypeParent(calleeExpression: KtExpression?): KtBinaryExpressionWithTypeRHS? {
        ...
        when (parent) {
            is KtBinaryExpressionWithTypeRHS -> return parent
            is KtQualifiedExpression -> return parent.parent.safeAs()
        }
        return null
    }
